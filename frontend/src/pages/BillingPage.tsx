...
import { generateBill, getBillForOrder, applyTip, splitBill, markBillPaid } from '../api/billing'
import { Bill, Order, Table } from '../types'
...
export default function BillingPage() {
  ...
  const genBill = useMutation({
    mutationFn: () => generateBill(latestOrder.id, strategy),
    onSuccess: (data) => { setBill(data); toast.success('Bill generated') },
    onError: (e: any) => toast.error(e.response?.data?.message ?? 'Failed to generate bill'),
  })

  const checkExistingBill = useMutation({
    mutationFn: () => getBillForOrder(latestOrder.id),
    onSuccess: (data) => { setBill(data); toast.success('Existing bill loaded') },
    onError: () => genBill.mutate(),
  })
  ...
  const tip = useMutation({
    ...
    onError: (e: any) => toast.error(e.response?.data?.message ?? 'Failed to apply tip'),
  })

  const split = useMutation({
    mutationFn: () => splitBill(bill!.id, parseInt(guestCount) || 1),
    onSuccess: (data) => { setBill(data); toast.success(`Split between ${guestCount} guests`) },
    onError: (e: any) => toast.error(e.response?.data?.message ?? 'Failed to split bill'),
  })
        ...
        <div className="card p-6 space-y-5">
          ...
          <div>
            ...
            {occupiedTables.length === 0 ? (
              <p className="text-sm text-slate-500">No tables with open orders.</p>
            ) : (
              <div className="space-y-2">
                {occupiedTables.map((t: Table) => (
                  ...
                  >
                    ...
                    <span className={clsx('badge', t.status === 'AWAITING_BILL' ? 'bg-amber-100 text-amber-700' : 'bg-orange-100 text-orange-700')}>
          ...
          <button
            disabled={!selectedTableId || !latestOrder || genBill.isPending || checkExistingBill.isPending}
            onClick={handleGenerateBill}
            className="btn-primary w-full flex items-center justify-center gap-2"
          >
            {(genBill.isPending || checkExistingBill.isPending) ? <Spinner size={16} /> : <Receipt size={15} />}
            Generate bill
          </button>
        </div>
        ...
        {bill ? (
          <div className="card p-6 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <span className={clsx('badge', bill.paid ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700')}>
            ...
            <div className="space-y-2">
              ...
              <div className="border-t border-slate-200 pt-3 space-y-1">
                ...
                {bill.tip > 0 && (
                  <div className="flex justify-between text-sm text-slate-600">
                    <span>Tip</span>
                    <span className="tabular-nums">£{bill.tip.toFixed(2)}</span>
                  </div>
                )}
              ...
              <>
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => tip.mutate()}
                      disabled={!tipAmount || tip.isPending}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={!guestCount || split.isPending}
                      className="btn-secondary whitespace-nowrap"
                    >
                      Split
                    </button>
                ...
                >
                  {pay.isPending ? <Spinner size={16} /> : <><CheckCircle2 size={16} /><CreditCard size={16} /></>}
                  Process payment
                </button>
              </>
            )}
